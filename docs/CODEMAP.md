...
| `src/App.tsx` | **Landing** (nav w/ anchor links, hero + badge + terminal demo, agents strip, how-it-works ×3, lab guarantees, CTA band, footer) and placeholder vault (signed in). **v6 API:** conditional rendering via `<Show when="signed-in|signed-out">` — `SignedIn`/`SignedOut` were removed in v6. All copy sourced from PLAN.md §1–2. | 🔜 Vault list wired to `GET /v1/traces` via Clerk JWT → API. Landing copy: "verification" described as part of the flow — matches Phase-2 plan; keep honest if messaging changes. |
