...
| File | Purpose | Review notes |
|---|---|---|
| `src/index.ts` | All routes: `GET /v1/health`; CORS (localhost:5173 + pages.dev origins); auth middleware on `/v1/*` (API keys for machines, skips `/v1/health` + `/v1/device/*`); device flow: `POST /v1/device/code` (public), `POST /v1/device/approve` (Clerk JWT, creates org-bound key, secret never shown in browser), `POST /v1/device/poll` (single-read handoff, 202 pending / 410 expired); `POST /v1/traces` (validate → sha256 idempotency → R2 put → D1 insert → 202); `GET /v1/traces`; `GET /v1/traces/:id`. | `trace_id`/`blob_key` format: `tr_<24hex>`, `traces/<org>/<id>.jsonl`. Duplicates return 200 + `duplicate: true`. Deployed: `opentraces-api.lalitmadan.workers.dev`. |
...
| `src/db/schema.sql` | D1/SQLite tables: `orgs`, `users`, `api_keys`, `sources`, `traces` (status enum in comment), `packs`, `pack_items`, `ledger_entries`, `device_auths` (device flow: pending → approved → consumed-on-poll). | Matches FLOW.md §8. `content_hash UNIQUE` is the idempotency guarantee. Applied to remote D1 `1dce496b-f006-4616-99f3-29d499575839`. Scrub pipeline is 🔜 next slice. |
...
| File | Purpose | Review notes |
|---|---|---|
| `opentraces/main.py` | Typer app: `ot version`; **`ot login`** — device flow by default (POST /v1/device/code → opens browser → polls until approved; key handed over via single-read poll, never on the clipboard), `--key` fallback for CI (paste + verify); `ot whoami`; `ot push --agent pi` (scan → rich table → selection `1,3-5|a` → upload with `Idempotency-Key`). | `--verbose` flag declared but not wired 🟡. Upload retries 3× on network errors (safe: content-hash idempotency prevents duplicates). Default is the real-user device path; CI users pass `--key`. |
...
| `src/App.tsx` | **Landing + routes**: tiny dependency-free router (`usePath`/`navigate`/`Link`/`Navigate`). Routes: `/` landing · `/sign-up` Clerk sign-up page · `/cli/auth` **device-approval page** (auto-opens sign-in modal when signed out, approves via `POST /v1/device/approve` with Clerk JWT, key goes only to the terminal) · `/dashboard` **live vault** (stat tiles + hairline trace table, rows clickable) · **`/dashboard/traces/:id` trace viewer**: header card (status, agent, model, steps, cost, repo@commit), timeline layout (number + role rail, hairline spine), 16px body text, thinking blocks (muted tinted), tool calls (dark terminal blocks, pretty-printed args), tool results (error-tinted), progressive rendering (50 at a time). Auth-aware CTAs; copy rules: customer-facing, zero em dashes. | API base from `VITE_API_URL` (baked at build). 🔜 Scrub report display once pipeline lands. |
